...
describe('GameService', () => {
  ...
  describe('Retire (明示的な離脱)', () => {
    ...
    it('プレイ中にリタイアすると猶予時間を待たず即座に死亡し、相手の勝利になること', () => {
      ...
      service.handleGameRetire('room-1', 'player-1', 'client-1');
    ...
    it('異なるクライアントからのリタイア要求は無視されること（ゴーストソケット対策）', () => {
      jest.advanceTimersByTime(GAME_COUNTDOWN_SEC * 1000);

      service.handleGameRetire('room-1', 'player-1', 'client-old');

      const session = roomsState.getRoom('room-1')?.gameSession;
      expect(session?.players['player-1'].alive).toBe(true);
    });
    ...
    it('waitingフェーズではリタイアしても死亡扱いにならないこと', () => {
      ...
      roomsState.addRoom(testRoom);
      service.handleGameJoin('room-waiting', 'player-3', 'client-3');

      service.handleGameRetire('room-waiting', 'player-3', 'client-3');
