...
export class FriendsController {
  ...
  @Get()
  friendsGet(@Request() req: UserRequest) {
  ...
  @Get('requests')
  friendsRequests(@Request() req: UserRequest) {
    const currentUserId = req.user.userId;
    return this.friendsService.getFriendsRequests(currentUserId);
  }

  @Put(':requestId/accept')
  friendsAccept(@Request() req: UserRequest, @Param('requestId') id: string) {
    const currentUserId = req.user.userId;
  ...
  friendsReject(@Request() req: UserRequest, @Param('requestId') id: string) {
    const currentUserId = req.user.userId;
    const friendshipId = id;
  ...
  @Delete(':userId')
  friendsDelete(@Request() req: UserRequest, @Param('userId') id: string) {
    const currentUserId = req.user.userId;
